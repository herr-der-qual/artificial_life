...
from src_grid.ui.inspector_panel import InspectorPanel
...
class View(arcade.View):
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if button == arcade.MOUSE_BUTTON_RIGHT:
            self._select_entity_at(x, y)

    def _select_entity_at(self, screen_x, screen_y):
        """Point query at the click, converted from screen to world space
        via the camera, then straight to a cell - no fuzzy radius needed
        like the pymunk system's SELECT_RADIUS, since a grid cell either
        ...
        self.inspector_panel.select(self.world.grid.occupant_at(col, row))
