...
class FileMenu:
    """Top-left "File" menu. Click to reveal a dropdown with Load / Save /
    Save As... Load opens a native file picker and forwards the chosen path
    to on_load. Save fires on_save directly. Save As... fires on_save_as,
    which the owner is expected to handle by opening a SaveAsDialog."""
    ...
    def __init__(self, on_load, on_save, on_save_as):
    ...
    def _build_widgets(self):
        ...
        self.save_as_button = arcade.gui.UIFlatButton(text="Save As...", width=button_width, height=self.BUTTON_HEIGHT)
    ...
    def _ask_for_file():
        ...
        try:
            path = filedialog.askopenfilename(
                title="Load World",
                filetypes=[("World config", "*.json"), ("All files", "*.*")],
            )
        finally:
            root.destroy()
        return path or None
